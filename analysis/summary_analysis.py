...
# Define the columns for the final summary DataFrame
...
class SummaryAnalysis:
    ...
    def __init__(self):
        """
        Initialize the SummaryAnalysis class.
        """
        # Initialize the final statistics DataFrame and lists to store analysis data
        self.final_statistics_df = pd.DataFrame(columns=FINAL_SUMMARY_DF_COLUMNS)
        self.all_confusion_matrix_analysis = list()
        self.all_test_cases_analysis = list()

    def add(self, confusion_matrix_analysis, test_cases_analysis):
        """
        Add confusion matrix and test cases analysis to their respective lists.

        :param confusion_matrix_analysis: Confusion matrix analysis object.
        :param test_cases_analysis: Test cases analysis object.
        """
    ...
    def finalize(self):
        """
        Process and compile the final statistics DataFrame.

        :return: Final statistics DataFrame.
        """
        for index, cma in enumerate(self.all_confusion_matrix_analysis):
            ...
            total_lost_classifications = (int(cma.total_truth_classification_high_priority_edits) +
                                          int(cma.total_truth_classification_low_priority_edits) -
            ...
            new_row = {
                ...
                'total_lost_classifications': total_lost_classifications,
            ...
            # Append the new row to the final statistics DataFrame
    ...
    def plot_metrics(self):
        """
        Plot performance metrics (accuracy, recall, precision, f1) by model.
        """
        # Pause execution for a short period
        self._pause_execution()
        # Prepare the grouped DataFrame for plotting
        df_grouped = self._prepare_grouped_df(['model', 'precision', 'f1', 'accuracy', 'recall'])
        ...
        self._create_bar_plot(df_melted, 'Performance Metrics by Model (Sorted by Recall, Precision, F1 and Accuracy)')

    def plot_lost_variance(self):
        """
        Plot the variance of lost classifications among rounds for each model.
        """
        # Pause execution for a short period
        self._pause_execution()
        # Prepare the filtered DataFrame for plotting
        df_filtered = self._prepare_filtered_df(['model', 'round', 'tn', 'fp', 'fn', 'tp',
                                                 'total_classifications', 'total_lost_classifications'])
        # Calculate additional columns for plotting
        df_filtered['avg_low_impact_predicted'] = df_filtered['tp'] + df_filtered['fn']
        df_filtered['avg_high_impact_predicted'] = df_filtered['tn'] + df_filtered['fp']
        df_filtered = df_filtered.rename(columns={'total_classifications': 'avg_classifications',
                                                  'total_lost_classifications': 'avg_lost_classifications'})
        ...
        # Create a box plot for the lost variance
        self._create_box_plot(df_filtered, 'Lost Variance Among Rounds for Each Model', 'avg_lost_classifications')
    ...
    def view_table_average_accuracy(self):
        """
        View the table with average accuracy by model.

        :return: DataFrame with average accuracy by model.
        """
        return self._view_table_average_metric('accuracy', 'average_accuracy')

    def view_table_average_recall(self):
        """
        View the table with average recall by model.

        :return: DataFrame with average recall by model.
        """
        return self._view_table_average_metric('recall', 'average_recall')

    def view_table_average_precision(self):
        """
        View the table with average precision by model.

        :return: DataFrame with average precision by model.
        """
        return self._view_table_average_metric('precision', 'average_precision')

    def view_table_average_f1(self):
        """
        View the table with average precision by model.

        :return: DataFrame with average precision by model.
        """
        return self._view_table_average_metric('f1', 'average_f1')

    def view_table_low_high_mixed_test_case_impacted(self):
        """
        View the table with low, high, and mixed impacted test cases by model and round.

        :return: DataFrame with low, high, and mixed impacted test cases.
        """
        # Prepare the filtered DataFrame for viewing
        df_filtered = self._prepare_filtered_df(['model', 'round', 'total_low_impacted', 'total_high_impacted',
                                                 'total_mixed_impacted', 'total_cts', 'perc_saved_cts'])
        # Format the percentage of saved test cases
        df_filtered['perc_saved_cts'] = df_filtered['perc_saved_cts'].map('{:.0%}'.format)
        # Drop the 'total_cts' column
    ...
    def _prepare_grouped_df(self, columns):
        """
        Prepare a grouped DataFrame for plotting.

        :param columns: List of columns to include in the grouped DataFrame.
        :return: Grouped DataFrame.
        """
        # Filter and copy the specified columns
        df_filtered = self.final_statistics_df[columns].copy()
        num_cols = columns[1:]
        ...
        return df_grouped.sort_values(by=num_cols, ascending=True)

    def _prepare_filtered_df(self, columns):
        """
        Prepare a filtered DataFrame for plotting.

        :param columns: List of columns to include in the filtered DataFrame.
    ...
    def _create_bar_plot(self, df, title):
        """
    ...
    def _create_box_plot(self, df, title, y_col):
        """
        ...
        # Create a box plot
        plt.figure(figsize=(6, 4))
        sns.boxplot(x='model', y=y_col, data=df)
        plt.title(title, fontsize=14)
        plt.xlabel('Text-generation LLM', fontsize=12)
    ...
    def _view_table_average_metric(self, metric, new_col_name):
        """
        View the table with average metric by model.

        :param metric: Metric to calculate the average for.
        :param new_col_name: New column name for the average metric.
        :return: DataFrame with average metric by model.
        """
        # Filter and copy the specified metric column
        df_filtered = self.final_statistics_df[['model', metric]].copy()
        # Group by 'model' and calculate the average metric
        grouped_df = df_filtered.groupby('model')[metric].mean().reset_index()
        # Sort the grouped DataFrame by the metric in descending order
        grouped_df = grouped_df.sort_values(by=metric, ascending=False)
        # Rename the metric column to the new column name
        grouped_df = grouped_df.rename(columns={metric: new_col_name})
        # Format the new column to 2 decimal places
        grouped_df[new_col_name] = grouped_df[new_col_name].map('{:.2f}'.format)
        # Convert the new column to float
        grouped_df[new_col_name] = grouped_df[new_col_name].astype(float)
        return grouped_df
